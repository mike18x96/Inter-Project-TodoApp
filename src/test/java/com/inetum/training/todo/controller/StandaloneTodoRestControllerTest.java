...
public class StandaloneTodoRestControllerTest {
    ...
    @BeforeEach
    public void setUp(){
        mockMvc = MockMvcBuilders.standaloneSetup(new TodoRestController(repository))
    ...
    public void getAll_TodoFound_returnsTodo() throws Exception {
        ...
        when(repository.findAll(any(Pageable.class))).thenReturn(new PageImpl<>(newArrayList(todo, todo2)));
        //when
        mockMvc.perform(MockMvcRequestBuilders.get(URL))
                .andDo(MockMvcResultHandlers.print())
        //then
                .andExpect(MockMvcResultMatchers.jsonPath("$.content", Matchers.hasSize(2)))
                .andExpect(MockMvcResultMatchers.jsonPath("$.content[0].name", Matchers.is("nazwa")))
                .andExpect(MockMvcResultMatchers.jsonPath("$.content[1].name", Matchers.is("nazwa2")));
    }
